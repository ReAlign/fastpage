...
let main = (fpConfig, conf) => {
    ...
    _keys.every(function(item) {
        if(item == 'mcss' && conf.mcssTopLevel) {
            _keysPath[item] = _folderPathArr.slice(0, 1);
        } else {
            if(item == 'jsCom') {
                _keysPath[item] = _folderPathArr.concat(['components']);
            } else {
                _keysPath[item] = _folderPathArr;
            }
        }
    ...
    let creatCpt = async () => {
        for(let i of _keys) {
            try {
                ...
                log.ok(`successfully created ` + i + `files`);
                file = {};
            } catch (err) {
                log.error(err);
            }
